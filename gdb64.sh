...
gdb -ex "target remote 127.0.0.1:1234" -ex "add-symbol-file ./build/kernel.elf ${ENTRY_ADDR}" -ex "b *KernelMain" -ex "layout split" -ex "add-symbol-file ./build/Loader.debug ${TEXT_ADDR} -s .data ${DATA_ADDR}" -ex "b *UefiMain" -ex "b *UefiMain+779"
