...
usb::EndpointConfig MakeEPConfig(const usb::EndpointDescriptor &ep_desc)
{
  ...
  conf.ep_id = usb::EndpointID{ep_desc.endpoint_address.bits.number, ep_desc.endpoint_address.bits.dir_in == 1};
...
usb::ClassDriver *NewClassDriver(usb::Device *dev, const usb::InterfaceDescriptor &if_desc)
{
  ...
  { // HID boot interface
    ...
    else if (if_desc.interface_protocol == 2)
    { // mouse
...
Error Device::StartInitialize()
{
  ...
  initialize_phase_ = 1;
  return GetDescriptor(*this, kDefaultControlPipeID, DeviceDescriptor::kType, 0, buf_.data(), buf_.size(), true);
}
...
Error Device::InitializePhase1(const uint8_t *buf, int len)
{
  ...
  return GetDescriptor(*this, kDefaultControlPipeID, ConfigurationDescriptor::kType, config_index_, buf_.data(),
                       buf_.size(), true);
}
...
Error GetDescriptor(Device &dev, EndpointID ep_id, uint8_t desc_type, uint8_t desc_index, void *buf, int len,
                    bool debug)
{
