...
namespace usb::cdc
{
CDCDriver::CDCDriver(Device *dev, const InterfaceDescriptor *if_comm, const InterfaceDescriptor *if_data)
    : ClassDriver{dev}, if_data_index_{if_data->interface_number}
{
}
...
Error CDCDriver::OnControlCompleted(EndpointID ep_id, SetupData setup_data, const void *buf, int len)
{
  Log(kDebug, "CDCDriver::OnControlCompleted: req_type=0x%02x req=0x%02x len=%u\n", setup_data.request_type.data,
      setup_data.request, len);
  return MAKE_ERROR(Error::kSuccess);
}
...
Error CDCDriver::SetLineCoding(const LineCoding &value)
{
  ...
  return ParentDevice()->ControlOut(kDefaultControlPipeID, setup_data, &line_coding_, sizeof(LineCoding), this);
}
} // namespace usb::cdc
