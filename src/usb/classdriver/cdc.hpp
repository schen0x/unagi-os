...
template <class T> const T *FuncDescDynamicCast(const uint8_t *desc_data)
{
  if (desc_data[1] == T::kType && desc_data[2] == static_cast<uint8_t>(T::kSubType))
  {
...
template <class T> T *FuncDescDynamicCast(uint8_t *desc_data)
{
...
{
public:
  CDCDriver(Device *dev, const InterfaceDescriptor *if_comm, const InterfaceDescriptor *if_data);
