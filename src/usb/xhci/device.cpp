...
void Log(LogLevel level, const DataStageTRB &trb)
{
  Log(level, "DataStageTRB: len %d, buf 0x%08lx, dir %d, attr 0x%02x\n", trb.bits.trb_transfer_length,
      trb.bits.data_buffer_pointer, trb.bits.direction, trb.data[3] & 0x7fu);
}
...
void Log(LogLevel level, const SetupStageTRB &trb)
{
  Log(level, "  SetupStage TRB: req_type %02x, req %02x, val %02x, ind %02x, len %02x\n", trb.bits.request_type,
      trb.bits.request, trb.bits.value, trb.bits.index, trb.bits.length);
}
...
void Log(LogLevel level, const TransferEventTRB &trb)
{
  if (trb.bits.event_data)
  {
    Log(level, "Transfer (value %08lx) completed: %s, residual length %d, slot %d, ep addr %d\n",
        reinterpret_cast<uint64_t>(trb.Pointer()), kTRBCompletionCodeToName[trb.bits.completion_code],
  ...
  Log(level, "%s completed: %s, residual length %d, slot %d, ep addr %d\n", kTRBTypeToName[issuer_trb->bits.trb_type],
      kTRBCompletionCodeToName[trb.bits.completion_code], trb.bits.trb_transfer_length, trb.bits.slot_id,
...
Error Device::ControlIn(EndpointID ep_id, SetupData setup_data, void *buf, int len, ClassDriver *issuer)
{
  ...
  if (buf)
  {
    auto setup_trb_position =
        TRBDynamicCast<SetupStageTRB>(tr->Push(MakeSetupStageTRB(setup_data, SetupStageTRB::kInDataStage)));
  ...
  {
    auto setup_trb_position =
        TRBDynamicCast<SetupStageTRB>(tr->Push(MakeSetupStageTRB(setup_data, SetupStageTRB::kNoDataStage)));
...
Error Device::ControlOut(EndpointID ep_id, SetupData setup_data, const void *buf, int len, ClassDriver *issuer)
{
  ...
  {
    auto setup_trb_position =
        TRBDynamicCast<SetupStageTRB>(tr->Push(MakeSetupStageTRB(setup_data, SetupStageTRB::kOutDataStage)));
