/**
 * The kutil function should work in both 32bit and 64bit
 */
 ...
 * e.g.: struct PATH_PART* pt_ptr[OS_PATH_MAX_LENGTH] = {0}; // (a line in the
 * stack)
 * */
...
int32_t to_digit(char c) { return c - 48; }
 ...
 * @ size_t @var{count}) Compares the first @var{count} bytes of two areas of
 * memory.  Returns zero if they are the same, a value less than zero if @var{x}
 * is lexically less than @var{y}, or a value greater than zero if @var{x} is
 * lexically greater than @var{y}.  Note that lexical order is determined as if
 * comparing unsigned char arrays.
 * @end deftypefn
 */
