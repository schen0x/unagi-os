...
- The exceptions are interrupts generated by **CPUs** when an 'error' occurs.
...
## M09 MEMORY_MALLOC_SBRK

- `brk()` can change the size of the data segment, thus can request more memory
...
### QEMU DEBUG INFO (CRASH LOG)

- [crash when loading PML4 to cr3 [paging], osdev](https://forum.osdev.org/viewtopic.php?f=1&t=40321)
...
- `v`: (the INT#/VECTOR in IVT) Exception Vector or Interrupt Vector; 0 indicates no exception; 0x0e is "Page Fault"
    - See "6.3.1 External Interrupts, Inter SDM 1-4, p3192"
- `e`: the Error Code
    - See "6.13 ERROR CODE, Inter SDM 1-4, p3208", 0010: EXT 0, software interrupt; IDT 0, TI 0, the index refers to a descriptor in GDT;
    - Which means the exception is caused by code in the SegmentSelector 0010
- `i`: bool; interrupt flag, 0 means interrupts is disabled disabled, 1 enabled
- `cpl`: Current Privilege Level. 0 being most privileged
- `IP`: Instruction Pointer; SegmentSelector:Offset
- `PC`: Program Counter; in x86_64 equals to RIP
- `SP`: Stack Pointer
- `CR2`: The `CR2` register, (x86_64 only); if a page fault exception occurs, `CR2` contains the linear address where the program attempted to access.
